...
python -c "from market_analyzer import SYMBOLS; print(' '.join(SYMBOLS[:10]))"
echo -e "\n-----------------------------------------"
...
while true; do
    ...
    case $option in
        ...
        2)
            ...
            python portfolio_assessor.py --portfolio
            ;;
        3)
            read -p "Enter Ticker Symbol (e.g. AMPX): " ticker
            if [ -n "$ticker" ]; then
                echo -e "\n${GREEN}Scanning Node: $ticker...${NC}"
                python portfolio_assessor.py --single "$ticker"
            else
